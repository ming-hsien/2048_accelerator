...
int main(int argc, char **argv)
{
    ...
    for (int count = 0; count < games; count++)
    {
        ...
        while (!game_over(my2048.board))
        {
            ...
            if (world_rank == 0)
            {
                recvbuf = (float *)malloc(sizeof(float) * world_size);
            }
            MPI_Gather(&score_local, 1, MPI_FLOAT, recvbuf, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
            ...
            MPI_Bcast(&best_move, 1, MPI_INT, 0, MPI_COMM_WORLD);

            if (world_rank > 0)
            {
                // int best_move_buf;
                // MPI_Recv(&best_move, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
